...
for ipath in dirs:
    if os.path.splitext(ipath)[1]=='.dbf':
        ...
        listNO2Temp=NO2AveSatelliteTemp.tolist()
        ...
        logNO2AveSatelliteTemp=np.log10(NO2AveSatelliteTemp)
        logPop2015TempForCalculate=np.log10(Pop2015TempForCalculate)
        # linear regression
        slope,intercept,rvalue,pvalue,stderr=stats.linregress(newDist, logNO2AveSatelliteTemp)
        
        eachFUAIntercept.append(intercept)
        eachLogFUAPop.append(logPop2015TempForCalculate)
        eachName.append(FuaNameTempShort)
        
        def myfunc(x):
            return slope*x+intercept
        getmodel=list(map(myfunc,newDist))
            
plt.scatter(eachLogFUAPop, eachFUAIntercept, marker='o',c='black',s=3)
# linear regression
slope1,intercept1,rvalue1,pvalue1,stderr1=stats.linregress(eachLogFUAPop, eachFUAIntercept)
slopePrintV=round(slope1,2)
InterceptPrintV=round(intercept1,2)
eestring='B = '+str(slopePrintV)+'lgP'+' - '+str(abs(InterceptPrintV))+", R\u00b2 = "+str(round(rvalue1*rvalue1,2))
...
for iia in range(len(eachName)):
    ax1.text(eachLogFUAPop[iia], eachFUAIntercept[iia], str(eachName[iia]), color='black',fontsize=5)
