...
for ipath in dirs:
    if os.path.splitext(ipath)[1]=='.dbf':
        ...
        logNO2AveSatelliteTemp1=np.log10(NO2AveSatelliteTemp1)
        # linear regression
        ...
        # collect slope
        eachFUASlopeHNO2.append(slope)
        eachLogFUAPop.append(logPop2015TempForCalculate)
        eachName.append(FuaNameTempShort)
        eachFUASlopeCBD.append(slopeCBD)
        Y=np.array(eachFUASlopeHNO2)-np.array(eachFUASlopeCBD)
...
eestring='Difference of slope (B) (HighestNO$_2$-Real CBD), \n'+'only exclude pixels with a distance of 0'
