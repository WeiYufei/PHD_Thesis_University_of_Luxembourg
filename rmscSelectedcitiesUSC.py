...
maxPop2015=max(np.round(pd.Series(dftableCell1Long.iloc[0:862225,14].tolist()).unique(),decimals=6))#to calculate the reference population and control precision
maxDistinM2=max(pd.Series(dftableCell1Long.iloc[0:862225,27].tolist()).unique())
maxDist=np.round((maxDistinM2/1000).tolist(),decimals=6)#find the reference distance and control precision

mergedCell = pd.read_csv("M://Eclipse_Workspace//Article2_Kriging_Py//MeCel1000GauKri_SelectedCitywMinFUA.csv",encoding = 'unicode_escape',low_memory = False)
...
logPop2015=np.round(dftableCell.iloc[0:104272,36].tolist(),decimals=6)
...
for ilz in range(len(indexHead)):
    ...
    ax1.text(distLessThr[5],NO2LessThr[5],FuaNameFullTemp, size=5, color=thisColorConfig)  
...
ax1.set_xlabel('R in km',fontsize=6.5)
ax1.set_ylabel('C(R) in mol/m$^2$',fontsize=6.5) 
...
#titleString="Mean of annual mean tropospheric NO$_2$ columns of selected FUA at rescaled distance (binned in "+str(binDist)+" km)"
...
legend_elements = [Line2D([0], [0], marker='o',linestyle='solid', label='Mean of C',color='black',markersize=0,linewidth=1.5),
                   gray_patch]
