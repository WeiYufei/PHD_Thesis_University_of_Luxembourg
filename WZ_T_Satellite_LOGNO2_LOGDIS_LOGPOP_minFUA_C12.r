...
data1 <- read.xlsx("M:/Eclipse_Workspace/Article1_NC_R/Regression_Cell_w_MinValue_equalCellDeleted.xlsx", sheet=1)
...
minFUAV<-data1$minFuaFul

logMeanNO2<-log(meanNO2,base=10)
logPopSize<-log(popSize15,base=10)
logDis<-log(distance,base=10)
logMinFUA<-log(minFUAV,base=10)


model1<-lm(logMeanNO2~logDis+logPopSize+logMinFUA)
summary(model1)
